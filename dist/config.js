...
function getParsedCommandLine(compiler, loaderOptions, configFilePath) {
    const result = compiler.getParsedCommandLineOfConfigFile(configFilePath, loaderOptions.compilerOptions, Object.assign({}, compiler.sys, { onUnRecoverableConfigFileDiagnostic: () => { } // tslint:disable-line no-empty
     }), extendedConfigCache);
    if (result) {
        const options = compilerSetup_1.getCompilerOptions(result);
        compiler.setConfigFileInOptions(options, result.options.configFile);
